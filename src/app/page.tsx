...
import { findSimilarFacts } from "@/lib/matchFactsFirst";
import FactsFirstSearch from "@/components/FactsFirstSearch";
...
export default function Home() {
  ...
  const similarFacts = findSimilarFacts(claim);
  interface Result {
    verdict: string;
    confidence: number;
    explanation?: string;
  ...
  const handleSubmit = async (
    ...
    try {
      ...
      const data: CheckClaimResponse = await response.json();
      ...
      const reportData: SubmitReportResponse = await reportResponse.json();
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <div className="bg-gradient-to-b from-blue-600 to-blue-400 text-white py-16">
        <div className="container mx-auto px-4 text-center">
          ...
          <p className="text-xl md:text-2xl max-w-3xl mx-auto opacity-90">
            Submit a claim or URL to check against trusted data sources and AI
            analysis
          </p>
        </div>
      </div>
      ...
      <FactsFirstSearch />
        <section className="max-w-3xl mx-auto glass-card rounded-lg shadow-lg p-6 md:p-8 mb-12 mt-6">
        ...
        {result && (
          <section className="max-w-3xl mx-auto glass-card rounded-lg shadow-lg p-6 md:p-8 mb-12">
            ...
            {/* FactFirstPH */}
            <a
              href={`https://factsfirst.ph/fact-checks?q=${encodeURIComponent(
                claim
              )}`}
              target="_blank"
            ...
            >
              Search more results on #FactsFirstPH
            </a>

            {similarFacts.length > 0 && (
              <div className="mt-4">
                <h3 className="text-sm font-semibold mb-2">
                  📰 Similar Fact Checks from #FactsFirstPH
                </h3>
                <ul className="space-y-1">
                  {similarFacts.map((fact, i) => (
                    <li key={i} className="bg-gray-50 p-2 border rounded">
                      <a
                        href={fact.link}
                        target="_blank"
                      ...
                      <p className="text-xs text-gray-600">{fact.summary}</p>
                    </li>
                  ))}
                </ul>
              </div>
            ...
            <div className="mb-6 text-xs text-gray-700 italic flex items-center">
              ...
              <p>
                Results are based on the Fake-News-Bert-Detect AI model. Higher
                confidence percentages generally indicate more reliable results.
              </p>
            </div>
